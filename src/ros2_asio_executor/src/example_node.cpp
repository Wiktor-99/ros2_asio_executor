...
public:
  AsioExampleNode(boost::asio::io_context& io_context)
    ...
    RCLCPP_INFO(this->get_logger(), "AsioExampleNode starting...");
        ...
        boost::asio::co_spawn(
          ...
          [this, service_handle, request_header]() -> boost::asio::awaitable<void> {
            ...
            try {
              ...
              RCLCPP_ERROR(this->get_logger(), "Async task failed: %s", ex.what());
              response->success = false;
              response->message = std::string("Task failed: ") + ex.what();
            }
